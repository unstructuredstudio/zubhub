    ...
    for each in subscribed:
        ...
        if each.creator.phone:
            phone_contexts.append(
                {"phone": each.creator.phone, "staff_pick_id": staff_pick.id}
            )
...
def filter_spam(ctx):
    site_url = (
        settings.DEFAULT_BACKEND_PROTOCOL + "://" + settings.DEFAULT_BACKEND_DOMAIN
    )
    ...
    if ctx.get("method") == "POST" and comment.publish.type != PublishingRule.DRAFT:
        ...
        if is_spam:
            ...
            # Set publisher_id to none when zubhub system unpublished a comment
            comment.publish.publisher_id = None
            comment.save()

            staffs_and_mods = Creator.objects.filter(is_staff=True)
            staffs_and_mods = staffs_and_mods.union(
...
def parse_comment_trees(user, data, creators_dict):
    ...
    def recursive_parse(data):
        ...
        for comment in data:
            ...
            # out all the places in the codebase where there are database query
            # related problems and start optimizing those queries.
...
def can_view(user, target):
    ...
    if (
        ...
        and (
            user.comments.filter(id=target.id).exists()
            or user.projects.filter(id=target.id).exists()
        )
    ):
        return True
    ...
    if (
        type == PublishingRule.PREVIEW
        and user.is_authenticated
        and (
            visible_to.filter(id=user.id).count() > 0
            or user.comments.filter(id=target.id).exists()
            or user.projects.filter(id=target.id).exists()
        )
    ):
        """
        Check if user is in visible_to or if project/comment
...
def get_published_projects_for_user(user, all):
    ...
    """
    fetch all projects where publishing rule is AUTHENTICATED_VIEWERS
    ...
    visible_to = all.filter(
        publish__type=PublishingRule.PREVIEW, publish__visible_to__id=user.id
    )

    if user.is_authenticated:
        published_by_owner = all.filter(
            publish__type=PublishingRule.PREVIEW,
            creator=user,
            publish__publisher_id=user.id,
        )
...
def detect_mentions(kwargs):
    ...
    if isinstance(text, str):
        mentions = list(
            map(
                lambda x: x.split("@")[1],
                re.findall("\B@[0-9a-zA-Z_.-]+", text),  # noqa: W605
            )
        )
...
default_search_criteria = {
    ProjectSearchCriteria.CATGEORY,
    ProjectSearchCriteria.TAG,
    ProjectSearchCriteria.TITLE_DESCRIPTION,
}


def perform_project_search(
    user, query_string, search_criteria: Optional[Set[ProjectSearchCriteria]] = None
):
    """
    ...
    if ProjectSearchCriteria.CATGEORY in search_criteria and result_categories_tree:
        ...
        for category in result_categories_tree:
            result_projects |= (
                category.projects.all().annotate(rank=rank).order_by("-rank")
            )
    #################################################################

    # fetch all projects whose tag(s) matches the search query
    result_tags = Tag.objects.filter(search_vector=query).prefetch_related("projects")
    ...
    # result_rules = PublishingRule.objects.filter(type=type).select_related("project_target") # noqa: E501
    ...
    if ProjectSearchCriteria.TITLE_DESCRIPTION in search_criteria:
        result_projects |= (
            Project.objects.annotate(rank=rank)
...
def recommend_projects(project):
    ...
    all_projects = Project.objects.filter(publish__type=PublishingRule.PUBLIC).exclude(
        pk=project.pk
    )
    ...
    projects = all_projects.filter(
        similar_title & (same_category | same_tags)
    ).distinct()
