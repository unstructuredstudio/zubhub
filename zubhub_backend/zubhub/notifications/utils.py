...
grouped_notification_types = {
    Notification.Type.BOOKMARK, Notification.Type.CLAP,
...
def push_notification(recipient, source, notification_type, link,
    ...
    if notification is not None:
        if notification.sources.filter(id=source.id).exists():
            return False
        ...
        notification.message = render_to_string(template_name, context).strip()
