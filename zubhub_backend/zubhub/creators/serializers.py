...
class CreatorMinimalSerializer(serializers.ModelSerializer):
    followers = serializers.SlugRelatedField(slug_field="id", read_only=True, many=True)
    ...
    badges = serializers.SlugRelatedField(
        slug_field="badge_title", read_only=True, many=True
    )
...
class AddGroupMembersSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if not group_members:
            raise serializers.ValidationError(
                _("You must submit at least one group member.")
            )
...
class CreatorGroupWithMembershipsSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        data["members"] = CreatorGroupMembershipSerializer(
            instance.memberships.all(), many=True
        ).data
        return data
    ...
    def update(self, instance, validated_data):
        ...
        for member_data in members_data:
            ...
            if member in existing_members:
                membership = CreatorGroupMembership.objects.get(
                    group=instance, member=member
                )
                membership.role = role
                membership.save()
            else:
                # If the member is not part of the group, add them with the specified role
                CreatorGroupMembership.objects.create(
                    group=instance, member=member, role=role
                )

        # Remove any members who were not included in the updated data
        members_to_remove = existing_members - set(
            [member_data["member"] for member_data in members_data]
        )
        CreatorGroupMembership.objects.filter(
            group=instance, member__in=members_to_remove
        ).delete()
