    ...
    <>
      {creators.length === 1 ? (
        creators.map(creator => (
          <Link
            to={{ pathname: `/creators/${creator.username}`, state: { prevPath: window.location.prevPath } }}
              ...
              <Tooltip title={creator.username} placement="bottom" arrow className={classes.creatorUsernameTooltip}>
                <Box>
                  ...
                  {creator.tags.map((tag, index) => (
                    <Typography key={index} color="textPrimary" className={classes.creatorTag} component="p">
                      {tag}
                    </Typography>
                  ))}
                </Box>
